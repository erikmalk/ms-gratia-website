...
import { homeMedia as fallbackHomeMedia, navigation as fallbackNavigation, portfolioCategories as fallbackCategories } from '@/lib/portfolio';
...
type AssetRow = {
  ...
  caption_text: string;
  caption_position: 'bottom-left' | 'bottom-right';
...
const toMediaItem = (row: AssetRow): MediaItem => ({
  ...
  caption: row.caption_text || undefined,
...
export const getCmsAssets = async (): Promise<CmsAsset[]> => {
  ...
  const rows = await sql<AssetRow[]>`
    select a.filename, a.src, a.alt_text, a.width, a.height, a.source_category,
      ...
      a.caption_text, a.caption_position, a.caption_color,
...
export const getHomeMedia = async (): Promise<MediaItem[]> => {
  if (!publicReadsEnabled()) return fallbackHomeMedia;
  try {
    const sql = getSql();
    const rows = await sql<AssetRow[]>`
      select a.filename, a.src, a.alt_text, a.width, a.height, a.source_category,
