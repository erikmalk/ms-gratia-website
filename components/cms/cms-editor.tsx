...
export function CmsEditor({ initialAssets, initialCategories }: { initialAssets: CmsAsset[]; initialCategories: CmsCategoryState[] }) {
  ...
  const visibleAssets = useMemo(() => {
    ...
    return assets.filter((asset) => (showArchived || !asset.archived) && (!query || `${asset.filename} ${asset.alt} ${asset.caption}`.toLowerCase().includes(query)));
  ...
  const updateCaption = (
    filename: string,
    change: Partial<Pick<CmsAsset, 'caption' | 'captionPosition' | 'captionColor'>>,
  ) => {
    setAssets((current) => current.map((asset) => asset.filename === filename ? { ...asset, ...change } : asset));
    markChanged();
  };
  ...
  const save = async () => {
    ...
    if (assets.some((asset) => !isHexColor(asset.captionColor))) {
      setStatus('error');
      setMessage('Caption colors must use a six-digit hex value, such as #ffffff.');
      return;
    }
    setStatus('saving');
    setMessage('');
    const response = await fetch('/api/cms/save', {
      ...
      body: JSON.stringify({
        ...
        captions: assets.map((asset) => ({
          ...
          text: asset.caption.trim(),
          position: asset.captionPosition,
          color: asset.captionColor.toLowerCase(),
        })),
          ...
          <div className="cms-grid" style={{ gridTemplateColumns: `repeat(auto-fill, minmax(${thumbSize}px, 1fr))` }}>
            {visibleAssets.map((asset) => (
              <article className={`cms-card${asset.archived ? ' is-archived' : ''}`} key={asset.filename}>
                ...
                {!asset.archived && asset.type === 'image' && <div className="cms-caption-controls">
                  <label>
                    Caption
                    <textarea
                      className="input"
                      ...
                      maxLength={500}
                      value={asset.caption}
                      onChange={(event) => updateCaption(asset.filename, { caption: event.target.value })}
                      placeholder="Optional explanation of the work"
                    />
                  </label>
                  <fieldset>
                    <legend>Position</legend>
                    <label><input type="radio" name={`caption-position-${asset.filename}`} checked={asset.captionPosition === 'bottom-left'} onChange={() => updateCaption(asset.filename, { captionPosition: 'bottom-left' })} /> Bottom left</label>
                    <label><input type="radio" name={`caption-position-${asset.filename}`} checked={asset.captionPosition === 'bottom-right'} onChange={() => updateCaption(asset.filename, { captionPosition: 'bottom-right' })} /> Bottom right</label>
                  </fieldset>
                  <label className="cms-caption-color">
                    Color
                    <input type="color" value={isHexColor(asset.captionColor) ? asset.captionColor : '#ffffff'} onChange={(event) => updateCaption(asset.filename, { captionColor: event.target.value })} />
                    <input className="input" value={asset.captionColor} maxLength={7} spellCheck={false} onChange={(event) => updateCaption(asset.filename, { captionColor: event.target.value })} aria-label={`Caption hex color for ${asset.filename}`} />
                  </label>
                </div>}
